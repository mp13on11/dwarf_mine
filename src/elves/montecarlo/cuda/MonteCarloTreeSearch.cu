...
__global__ void computeSingleMove(curandState* deviceStates, Field* playfield, size_t moveX, size_t moveY, int directionX, int directionY, Player currentPlayer);
...
void compute(size_t reiterations, size_t dimension, Field* playfield, size_t* moveX, size_t* moveY, size_t* wins, size_t* visits)
{
    ...
    computeSingleMove <<< NUMBER_OF_BLOCKS, THREADS_PER_BLOCK >>>(deviceStates, playfield, 1U, 1U, 1, 1, White);
